...
class VerifyEmailJobTest extends TestCase
{
    ...
    public function test_handle_persists_result()
    {
        ...
        $mock->shouldReceive('verifySMTP')->once()->with($email, ['mx1.example.com'])->andReturn(['smtp' => 'valid']);
        $mock->shouldReceive('detectCatchAll')->once()->with($email, ['mx1.example.com'])->andReturn(false);
        ...
        $job = new VerifyEmailJob(null, $email);
        $job->handle();
